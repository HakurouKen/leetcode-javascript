...
和[026. Remove Duplicates from Sorted Array](./026.%20Remove%20Duplicates%20from%20Sorted%20Array.md) 整体的思想类似，也是采取双指针来实现。唯一不同的是，在比较的时候，需要满足 `nums[i] = nums[count] = nums[count-1]` 的条件才跳过。在实际计算中，由于数组已经排序，我们可以直接比较 `nums[i]` 和 `nums[count-1]`是否相等即可。
